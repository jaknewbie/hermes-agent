    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            if _lpt:
                ...
                # Persist current context fill to a file so external tooling
                # (e.g. the agent's status_context.py footer) can read it without
                # live access to the gateway process. Written on every /usage call.
                try:
                    import json as _json
                    ...
                    _Path(os.path.expanduser("~/.hermes/context_status.json")).write_text(
                        _json.dumps({
                            "context_used": int(_lpt),
                            "context_max": int(ctx.context_length or 0),
