...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # FIX (jak-local-mods): clear any stale _send_path_degraded flag
            # on a successful (re)connect. The flag is set True when a polling
            # generation starts (adapter.py:2046) and only cleared by
            # _record_polling_progress() (adapter.py:2060), which depends on the
            # getUpdates progress observer firing. If that observer never fires
            # (e.g. transient network states, reconnect races), the flag stays
            # True forever and EVERY outbound message is silently dropped
            # (send_path_degraded) — making /restart and all replies look like
            # they "do nothing" even though the gateway is healthy. Resetting
            # here on a confirmed connect guarantees the send path is usable
            # as soon as Telegram is reachable again, independent of the
            # progress observer. (#jak-send-path-stuck)
            if getattr(self, "_send_path_degraded", False):
                self._send_path_degraded = False
                logger.info(
                    "[%s] Cleared stale _send_path_degraded after connect (%s mode) — send path restored",
                    self.name,
                    mode,
                )
